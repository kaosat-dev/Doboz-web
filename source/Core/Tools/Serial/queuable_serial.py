...
#module_logger = logging.getLogger("hydroduino.arduino_manager.Serial.eventSerial")

class SerialEvents(Events):
    """
    Class defining events associated to Event Serial Class
    """
    __events__=('OnDataRecieved', 'OnPortDisconnected', 'OnPortReconnected')
...
class serial(Thread):
    ...
    def connect(self):
        ...
        while self.port is None and self.currentErrors<5:
            try:      
                ...
                serial.blockedPorts.append(self.port)        
                ...
                self.logger.critical("cricital error while (re-)starting serial connection")
    ...
    def scan(self):
        ...
        for port in self.list_ports():  
            if port not in serial.blockedPorts: 
                try:
                    s = Serial(port) 
